...
included_dirs="storage_gateway_dashboard"
...
function run_pep8 {
  ...
  DJANGO_SETTINGS_MODULE=storage_gateway_dashboard.test.settings ${command_wrapper} flake8
}
...
function run_pep8_changed {
    ...
    warn_on_flake8_without_venv
    diff -u --from-file /dev/null ${files} | DJANGO_SETTINGS_MODULE=storage_gateway_dashboard.test.settings ${command_wrapper} flake8 --diff
    exit
}

function run_sphinx {
    echo "Building sphinx..."
    DJANGO_SETTINGS_MODULE=storage_gateway_dashboard.test.settings ${command_wrapper} python setup.py build_sphinx
    echo "Build complete."
}
...
function run_tests {
  ...
  if [ $with_selenium -eq 0 -a $integration -eq 0 ]; then
      testopts="$testopts --exclude-dir=storage_gateway_dashboard/test/integration_tests"
  fi
...
function run_tests_all {
  echo "Running Storage_gateway Dashboard tests"
  export NOSE_XUNIT_FILE=storage_gateway_dashboard/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='dashboard_nose_results.html'
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test storage_gateway_dashboard --settings=openstack_dashboard.test.settings $testopts
...
function run_makemessages {
  ...
  echo -n "storage_gateway dashboard: "
  cd storage_gateway_dashboard
  babel_extract django
  STORAGE_GATEWAY_PY_RESULT=$?

  echo -n "storage_gateway dashboard javascript: "
  babel_extract djangojs
  STORAGE_GATEWAY_JS_RESULT=$?

  cd ..
  if [ $check_only -eq 1 ]; then
    rm storage_gateway_dashboard/locale/django*.pot
  fi

  exit $(($STORAGE_GATEWAY_PY_RESULT || $STORAGE_GATEWAY_JS_RESULT))
}
