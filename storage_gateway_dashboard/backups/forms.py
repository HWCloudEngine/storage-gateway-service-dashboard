...
class CreateBackupForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        ...
        try:
            backup = sg_api.volume_backup_create(request,
                                                 data['volume_id'],
                                                 data['name'],
                                                 data['description'])
...
class RestoreBackupForm(forms.SelfHandlingForm):
    ...
    def get_cinder_volumes(self, request):
        volumes = []
        try:
            available = cinder.VOLUME_STATE_AVAILABLE
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        try:
            volumes = self.get_cinder_volumes(request)
        except Exception:
    ...
    def handle(self, request, data):
        ...
        try:
            restore = sg_api.volume_backup_restore(request,
                                                   backup_id,
                                                   volume_id)
