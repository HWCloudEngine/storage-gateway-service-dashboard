...
BACKUP_DELETABLE_STATES = ("available", "error")
...
class DeleteBackup(tables.DeleteAction):
    ...
    def delete(self, request, obj_id):
        sg_api.volume_backup_delete(request, obj_id)

    def allowed(self, request, backup=None):
        if backup:
            return backup.status in BACKUP_DELETABLE_STATES
        return True
...
class RestoreBackup(tables.LinkAction):
    ...
    def get_link_url(self, datum):
        ...
        url = reverse("horizon:storage-gateway:backups:restore",
                      args=(backup_id,))
...
class BackupsTable(tables.DataTable):
    ...
    name = tables.Column("name",
                         verbose_name=_("Name"),
                         link="horizon:storage-gateway:backups:detail")
    ...
    volume_name = BackupVolumeNameColumn("name", verbose_name=_("Volume Name"),
                                         link="horizon:storage-gateway:"
                                              "volumes:detail")

    class Meta(object):
        name = "volume_backups"
        verbose_name = _("Storage Gateway Volume Backups")
