...
class CreateBackupView(forms.ModalFormView):
    ...
    template_name = 'backups/create_backup.html'
    submit_label = _("Create Storage Gateway Volume Backup")
    submit_url = "horizon:storage-gateway:volumes:create_backup"
    success_url = reverse_lazy("horizon:storage-gateway:backups:index")
    page_title = _("Create Storage Gateway Volume Backup")
...
class BackupDetailView(tabs.TabView):
    ...
    def get_data(self):
        try:
            ...
            backup = sg_api.volume_backup_get(self.request, backup_id)
        except Exception:
    ...
    def get_redirect_url():
        return reverse('horizon:storage-gateway:backups:index')


class RestoreBackupView(forms.ModalFormView):
    form_class = backup_forms.RestoreBackupForm
    template_name = 'backups/restore_backup.html'
    submit_label = _("Restore Backup to Volume")
    submit_url = "horizon:storage-gateway:backups:restore"
