...
class CreateForm(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            ...
            master_vol = data.get('master_volume', None)
            slave_vol = data.get('slave_volume', None)

            if master_vol == slave_vol:
                error_message = (_('The slave volume and master volume can not'
                                   'be the same'))
                raise ValidationError(error_message)

            replication = sg_api.volume_replication_create(
                    request, master_vol, slave_vol, name, description)
...
class RollbackForm(forms.SelfHandlingForm):
    checkpoint = forms.ThemableChoiceField(label=_("Rollback Replication"),
                                           help_text=_(
                                                   "Select an checkpoint to "
                                                   "rollback."))
