...
DELETABLE_STATES = ("available", "error", "enabled")
...
class DisableVolume(tables.BatchAction):
    name = "disable"
    help_text = _("The data will remain in the volume but storage gateway"
                  " service will be disabled to the volume")
    action_type = "danger"

    @staticmethod
    def action_present(count):
        return npgettext_lazy(
            "Action to perform (the volume is currently enabled)",
            u"Disable Volume",
            u"Disable Volumes",
            count
        )
    ...
    def action_past(count):
        return npgettext_lazy(
            "Past action (the volume is currently being disabled)",
            u"Disabling Volume",
            u"Disabling Volumes",
            count
        )
    ...
    def allowed(self, request, volume):
        if volume:
            return volume.status in ["enabled"]
        return True
    ...
    def allowed(self, request, volume=None):
        ...
        if volume:
            ...
            if attach_allowed or detach_allowed:
                return volume.status in ("available", "in-use", "enabled")
        return False
...
class VolumesTable(VolumesTableBase):
    ...
    replication_status = tables.Column(
            "replication_status", verbose_name=_(" Replication Status"),
            status=True, status_choices=VolumesTableBase.STATUS_CHOICES,
            display_choices=VolumesTableBase.STATUS_DISPLAY_CHOICES)

    class Meta(object):
        name = "volumes"
        verbose_name = _("Storage Gateway Volumes")
        status_columns = ["status", "replication_status"]
        row_class = UpdateRow
